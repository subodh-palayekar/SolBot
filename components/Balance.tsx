...
const Balance = ({ accountId }: BalanceParamsType) => {
  ...
  const [cachedAccountDetails, setCachedAccountDetails] = useState<
    Record<string, WalletType>
  >({});
  const [accountDetails, setAccountDetails] = useState<WalletType | undefined>(
    undefined
  );
  ...
  const getDetails = async (coin: string) => {
    setIsLoading(true);

    // Check if the details for this coin are already cached
    if (cachedAccountDetails[coin]) {
      setAccountDetails(cachedAccountDetails[coin]);
      setIsLoading(false);
      return;
    }
    ...
    const walletType = coin === 'Sol' ? 'solWallet' : 'ethWallet';
    const { privateKey, publicKey, balance, balanceInUsd } =
      accountDetails?.[walletType];

    const walletData = { privateKey, publicKey, balance, balanceInUsd };

    // Cache the fetched details for the selected coin
    setCachedAccountDetails((prev) => ({ ...prev, [coin]: walletData }));
    setAccountDetails(walletData);
    setIsLoading(false);
  };
      ...
      <div className="dark:bg-[#040307] bg-[#c0b3fb] p-4 rounded-2xl">
        ...
        <div className="flex justify-center items-center gap-2 flex-wrap">
          ...
          <span className="font-semibold text-[40px] sm:text-[60px] text-black dark:text-white">
            {parseFloat(accountDetails?.balanceInUsd || '0').toFixed(2)}
          </span>
        </div>
          ...
          <div className="flex items-center gap-3 px-4 sm:px-8 py-4">
            ...
            <div className="flex w-full items-center justify-between">
              <span className="text-black dark:text-white">
                {parseFloat(accountDetails?.balance || '0').toFixed(8)} {coin}
              </span>
              <span
                className={cn(
                  'p-2 rounded-full',
